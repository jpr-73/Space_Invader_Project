...
class Game1{
  ...
  private val RESPAWN_DELAY_MS = 3000 // 5 seconds
  ...
  // Store dead invader info as tuples: (originalX, originalY, size, color, deathTime)
  ...
  private val scoreFont = new java.awt.Font("Lithograph", java.awt.Font.BOLD, 36)
  ...
  private def drawStartScreen(): Unit = {
    ...
    val titleFont = new java.awt.Font("Lithograph", java.awt.Font.BOLD, 72)
    ...
    // Draw button text
    val buttonFont = new java.awt.Font("Lithograph", java.awt.Font.BOLD, 36)
    display.drawString(buttonX + 80, buttonY + 50, "START", buttonFont, Color.BLACK)

    // Draw instructions
    val instructFont = new java.awt.Font("Arial", java.awt.Font.PLAIN, 24)
  ...
  private def respawnInvaders(): Unit = {
    ...
    for (idx <- deadInvaders.indices) {
      val (origX, origY, size, color, deathTime) = deadInvaders(idx)

      if (currentTime - deathTime >= RESPAWN_DELAY_MS) {
        i += new Invader(origX, origY, size, color)
        toRemove += idx
      }
    }
  ...
  private val timer = new Timer(16, (_: ActionEvent)=>{
    ...
    // If game hasn't started, just show start screen
    if (!gameStarted) {
      drawStartScreen()
    }else {

      // Game logic only runs after start button is pressed
      if (firstDraw) {
      ...
      for ((proj, pIdx) <- ship_proj.zipWithIndex) {
        for ((invader, iIdx) <- i.zipWithIndex) {
          if (
            ...
            // Save invader info as a tuple before removing
            deadInvaders += ((invader.x, invader.y, invader.size, invader.c, System.currentTimeMillis()))
      ...
      display.drawString(20, 60, s"Lives : $numLives",scoreFont ,Color.WHITE)
    }
  })
  timer.start()
