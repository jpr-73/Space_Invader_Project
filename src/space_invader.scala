...
class Game1 {
  ...
  private val starX = Array.fill(starCount)(scala.util.Random.nextInt(screenWidth ))
  private val starY = Array.fill(starCount)(scala.util.Random.nextInt(screenHeight))
  ...
  private def moveship(): Unit = {
    ...
    spaceship.x = Math.max(0, Math.min(screenWidth - spaceship.size, nextX))
    spaceship.y = Math.max(0, Math.min(screenHeight - spaceship.size, nextY))
  ...
  private def moveInvader(): Unit = {
    ...
    if(rightMost >= screenWidth) invaderDir = -1
  ...
  private def projectiltrajectory(): Unit = {
    ...
    while(i < shipProjCount){
      val p = shipProj(i)
      p.update()
      if(p.bounds(screenWidth, screenHeight)){
        // remove projectile
